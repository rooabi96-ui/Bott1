...
TASKS_PER_DAY = int(os.getenv("TASKS_PER_DAY", "3"))  # incluye campaña si hay
LEVEL_STEP_DAYS = 4
MAX_LEVEL = 4
...
def back_to_menu_btn(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Volver al menú", callback_data="menu:home")]])
...
# ======================
# CAMPAIGNS / PAYOUT (pago automático al confirmar)
# ======================
...
def compute_campaign_payout(conn, campaign_id: int, user_level: int) -> int:
    ...
    # pago base para poder llegar al objetivo
    base = max(1, remaining_budget // remaining_needed)

    # bonus suave por nivel (0..3)
    bonus = max(0, min(user_level - 1, 3))
    desired = base + bonus

    # asegurar que siempre quede para el resto
...
async def show_admin_panel(q):
    ...
    await q.edit_message_text(
        ...
        "Desde acá podés cargar tareas y campañas.",
...
async def retirar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status == "pending_exists":
        await update.message.reply_text(
            ...
            "Enviame tus datos (un solo mensaje):\n"
    ...
    await update.message.reply_text(
        ...
        "Ahora enviame tus datos (un solo mensaje):\n"
        "**Alias / CBU / Banco / Titular / DNI**",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=inline_menu(u.id)
    )
...
async def handle_task_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # LINK / CAMPAIGN: CONFIRMAR (SIN CÓDIGO)
...
async def handle_admin_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin:list_campaigns":
        ...
        for c in rows:
            lines.append(
                f"• #{c['id']} {c['name']} | Pres: {format_usd_from_cents(c['budget_usd_cents'])} | "
                f"Obj: {c['goal_completions']} | Done: {c['completed_count']} | "
                f"Gasto: {format_usd_from_cents(c['spent_usd_cents'])} | {'✅' if c['is_active'] else '⛔'}"
            )
        await q.edit_message_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN, reply_markup=admin_panel_kb())
        return
...
# ======================
# TEXT INPUT (quiz / retiro / admin flows)
# ======================
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(pending_quiz, dict):
        ...
        out = "✅ Quiz completado." if inserted else "✅ Ya estaba completado."
        if streaked:
            out += f"\n🔥 Racha: **{streak}** | 🏅 Nivel: **{lvl}**"
