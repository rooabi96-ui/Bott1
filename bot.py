...
# Niveles: sube 1 nivel cada 4 días de racha COMPLETA
LEVEL_STEP_DAYS = 4
MAX_LEVEL = 10

# Retiro
MIN_WITHDRAW_USD_CENTS = 500  # $5.00
MIN_PAYOUT_USD_CENTS = 1      # piso pago campaña
...
def is_admin(user_id: int) -> bool:
    # Si ADMIN_IDS está vacío, deja todo como admin (útil para pruebas).
...
def main_menu(user_is_admin: bool) -> ReplyKeyboardMarkup:
    rows = [
        ["📅 Tareas", "💰 Saldo"],
        ["🏅 Nivel", "🎁 Código"],
        ["💸 Retirar", "📜 Historial"],
    ...
    if user_is_admin:
        rows.append(["🧩 Catálogo", "🎯 Campañas"])
        rows.append(["➕ Crear tarea", "➕ Crear campaña"])
        rows.append(["💳 Retiros", "📊 Stats"])
        ...
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                ...
                CREATE TABLE IF NOT EXISTS daily_tasks (
                    ...
                    payload TEXT,                -- quiz: answer=... | link/campaign: code=...
                    link_url TEXT,
                    UNIQUE(day, idx)
                );
                """)
                ...
                CREATE TABLE IF NOT EXISTS withdrawals (
                    ...
                    payout_details TEXT,         -- alias/cbu/cuenta/etc
...
# ========= CAMPAIGN PAYOUT (auto-ajuste por presupuesto/objetivo + nivel suave) =========
...
# ========= WITHDRAWALS (DISPONIBLE vs RETENIDO) =========
...
def attach_withdrawal_details(conn, user_id: int, details: str):
    ...
    conn.commit()
    return wrow  # {id, amount_usd_cents}
...
async def tareas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _work():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            ...
            if not daily_tasks_exist(conn, day):
                create_daily_tasks(conn, day, TASKS_PER_DAY)
            user = get_user(conn, u.id)
            tasks = list_daily_tasks(conn, u.id, day)
            active_campaign = get_active_campaign(conn)
            return user, tasks, active_campaign

    user, tasks, active_campaign = await asyncio.to_thread(_work)

    if not tasks:
        await update.message.reply_text(
            "⚠️ Hoy no hay tareas publicadas.\n\nSi sos admin: cargá tareas con ➕ Crear tarea.",
            reply_markup=main_menu(is_admin(u.id))
        )
        return
    ...
    if active_campaign:
        def _payout_preview():
            with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
                return compute_campaign_payout(conn, active_campaign["id"], user["level"])
        preview = await asyncio.to_thread(_payout_preview)

        lines.append(
            f"🎯 **Campaña activa:** {active_campaign['name']}\n"
            f"📌 Progreso: {active_campaign['completed_count']}/{active_campaign['goal_completions']}\n"
...
async def saldo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu(is_admin(u.id)))
...
async def nivel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"👉 Próximo objetivo: llegar a **{next_level_at}** días."
    )
    await update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu(is_admin(u.id)))
...
async def ayuda(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "Tocá **🎁 Código** para ingresarlo.\n\n"
        f"💸 Retiro mínimo: **{format_usd_from_cents(MIN_WITHDRAW_USD_CENTS)}**"
    )
    await update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu(is_admin(update.effective_user.id)))
...
async def checkin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status == "no_task":
        await update.message.reply_text("Hoy no hay check-in 🤍", reply_markup=main_menu(is_admin(u.id)))
        return
...
async def quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status, extra = await asyncio.to_thread(_work)
    if status == "no_task":
        await update.message.reply_text("Hoy no hay quiz 🤍", reply_markup=main_menu(is_admin(u.id)))
        return
    if status == "need":
        await update.message.reply_text(f"Escribí: /quiz {extra}", reply_markup=main_menu(is_admin(u.id)))
        return
    if status == "wrong":
        await update.message.reply_text(f"❌ Incorrecto. Probá: /quiz {extra}", reply_markup=main_menu(is_admin(u.id)))
        return
...
async def go(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def maybe_notify_withdrawable(user_id: int):
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            ...
            # SOLO SALDO DISPONIBLE cuenta para retiro
        ...
        await context.bot.send_message(
            ...
            text=(
                ...
                "Si querés, tocá **💸 Retirar** para solicitar tu pago."
            ),
            parse_mode=ParseMode.MARKDOWN,
    ...
    status, extra = await asyncio.to_thread(_work)
    if status == "no_task":
        await update.message.reply_text("Hoy no hay códigos 🤍", reply_markup=main_menu(is_admin(u.id)))
        return
    if status == "need":
        await update.message.reply_text(f"Ingresá: /go {extra}", reply_markup=main_menu(is_admin(u.id)))
        return
    if status == "wrong":
        await update.message.reply_text("❌ Código incorrecto. Mirá el correcto en 📅 Tareas.", reply_markup=main_menu(is_admin(u.id)))
        return
...
async def retirar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _work_create():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            ...
            # SOLO SALDO DISPONIBLE cuenta para pedir retiro
            if user["balance_usd_cents"] < MIN_WITHDRAW_USD_CENTS:
                return ("not_enough", None, user["balance_usd_cents"], user.get("held_usd_cents", 0))

            # Retira TODO el saldo disponible (lo pasa a retenido)
    ...
    if status == "pending_exists":
        await update.message.reply_text(
            ...
            "**Alias / CBU / Banco / Titular / DNI** (lo que tengas).",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=main_menu(is_admin(u.id))
        )
        return
    ...
    await update.message.reply_text(
        ...
        f"🔒 Ahora tu saldo pasó a **RETENIDO** hasta que se pague.\n\n"
        f"💰 Disponible: **{format_usd_from_cents(available)}**\n"
        f"🔒 Retenido: **{format_usd_from_cents(held)}**\n\n"
...
async def historial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _work():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            ensure_user(conn, u)
            items = list_withdrawals_user(conn, u.id, limit=10)
            return items

    items = await asyncio.to_thread(_work)
    if not items:
        await update.message.reply_text("📜 Todavía no tenés retiros.", reply_markup=main_menu(is_admin(u.id)))
        return
    ...
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu(is_admin(u.id)))


async def capture_withdraw_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
...
# ========= ADMIN: ver/pagar/rechazar retiros =========
async def withdrawals_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    ...
    def _work():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            return list_withdrawals_pending(conn, limit=30)

    items = await asyncio.to_thread(_work)
    if not items:
        await update.message.reply_text("✅ No hay retiros pendientes.", reply_markup=main_menu(True))
        return
    ...
    lines.append("\nMarcar pagado: /paywithdraw <id> (opcional nota)\nRechazar: /rejectwithdraw <id> <motivo>")
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu(True))
...
async def paywithdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /paywithdraw <id> (opcional: nota)", reply_markup=main_menu(True))
        return
    ...
    def _work():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id, user_id, amount_usd_cents, status
                    FROM withdrawals
                    WHERE id=%s
                """, (wid,))
                w = cur.fetchone()
    ...
    if status == "not_found":
        await update.message.reply_text("⚠️ No existe ese retiro.", reply_markup=main_menu(True))
        return
    if status == "not_pending":
        await update.message.reply_text("⚠️ Ese retiro no está en estado pendiente.", reply_markup=main_menu(True))
        return
    if status == "insufficient_held":
        await update.message.reply_text("⚠️ No hay saldo retenido suficiente para ese retiro.", reply_markup=main_menu(True))
        return
    if status != "ok":
        await update.message.reply_text("⚠️ No pude completar la operación.", reply_markup=main_menu(True))
        return

    await update.message.reply_text("✅ Marcado como pagado y saldo retenido descontado.", reply_markup=main_menu(True))
...
async def rejectwithdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /rejectwithdraw <id> <motivo>", reply_markup=main_menu(True))
        return
    ...
    if status == "not_rejectable":
        await update.message.reply_text("⚠️ Ese retiro no se puede rechazar en su estado actual.", reply_markup=main_menu(True))
        return
    if status != "ok":
        await update.message.reply_text("⚠️ No pude completar la operación.", reply_markup=main_menu(True))
        return

    await update.message.reply_text("✅ Rechazado y saldo devuelto al disponible.", reply_markup=main_menu(True))
...
# ========= ADMIN: tareas/campañas/stats =========
async def ayuda_addtask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "➕ **Crear tarea (admin)**\n\n"
...
async def ayuda_campaign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "➕ **Crear campaña (admin)**\n\n"
...
async def tasksadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    items = await asyncio.to_thread(_work)
    if not items:
        await update.message.reply_text("📦 Catálogo vacío. Tocá ➕ Crear tarea.", reply_markup=main_menu(True))
        return
...
async def addtask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" not in text:
        return await ayuda_addtask(update, context)
    try:
        ...
        await update.message.reply_text(f"✅ Tarea agregada. ID: {new_id}", reply_markup=main_menu(True))
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}", reply_markup=main_menu(True))
...
async def deltask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(u.id):
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /deltask <id>", reply_markup=main_menu(True))
        return
    ...
    await update.message.reply_text("✅ Eliminada." if deleted else "⚠️ No encontré ese ID.", reply_markup=main_menu(True))
...
async def toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(u.id):
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /toggle <id>", reply_markup=main_menu(True))
        return
    task_id = int(context.args[0])
    def _work():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
    ...
    await update.message.reply_text(("✅ Activada." if state else "⛔ Pausada.") if state is not None else "⚠️ No encontré ese ID.", reply_markup=main_menu(True))
...
async def newcampaign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" not in text:
        return await ayuda_campaign(update, context)
    try:
        ...
        await update.message.reply_text(f"✅ Campaña creada. ID: {cid}", reply_markup=main_menu(True))
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}", reply_markup=main_menu(True))
...
async def campaigns(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    items = await asyncio.to_thread(_work)
    if not items:
        await update.message.reply_text("No hay campañas.", reply_markup=main_menu(True))
        return
...
async def endcampaign_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(u.id):
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /endcampaign <id>", reply_markup=main_menu(True))
        return
    ...
    await update.message.reply_text("✅ Cerrada." if changed else "⚠️ No existe.", reply_markup=main_menu(True))
    ...
    def _work():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT COUNT(*) AS n FROM task_catalog")
                catalog_n = cur.fetchone()["n"]
                cur.execute("SELECT COUNT(*) AS n FROM campaigns")
                camp_n = cur.fetchone()["n"]
                cur.execute("SELECT COUNT(*) AS n FROM withdrawals WHERE status='pending'")
                pending_w = cur.fetchone()["n"]
...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Si está esperando que mande datos bancarios, capturamos cualquier texto "libre"
    def _has_pending():
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            user = get_user(conn, u.id)
            return bool(user and user["pending_withdraw_id"])
    has_pending = await asyncio.to_thread(_has_pending)

    if has_pending:
        known_buttons = {
            "📅 Tareas","💰 Saldo","🏅 Nivel","🎁 Código","💸 Retirar","📜 Historial","ℹ️ Ayuda",
            "🧩 Catálogo","🎯 Campañas","➕ Crear tarea","➕ Crear campaña","💳 Retiros","📊 Stats"
        }
        if text not in known_buttons:
            return await capture_withdraw_details(update, context)
    ...
    if text == "🏅 Nivel":
        return await nivel(update, context)
    if text == "🎁 Código":
        return await ask_code(update, context)
    if text == "💸 Retirar":
        return await retirar(update, context)
    if text == "📜 Historial":
        return await historial(update, context)
    if text == "ℹ️ Ayuda":
    ...
    if is_admin(u.id):
        if text == "🧩 Catálogo":
            return await tasksadmin(update, context)
        if text == "➕ Crear tarea":
            return await ayuda_addtask(update, context)
        if text == "🎯 Campañas":
            return await campaigns(update, context)
        if text == "➕ Crear campaña":
            return await ayuda_campaign(update, context)
        if text == "💳 Retiros":
            return await withdrawals_admin(update, context)
        if text == "📊 Stats":
            return await stats(update, context)

    await update.message.reply_text("Tocá un botón del menú 👇", reply_markup=main_menu(is_admin(u.id)))
...
def main():
    ...
    app.add_handler(CommandHandler("withdrawals", withdrawals_admin))
