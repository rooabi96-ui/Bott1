...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    "✅ Eliminada." if eliminado else "⚠️ No encontré ese ID.",
    reply_markup=inline_menu(update.effective_user.id)
)
...
async def addtask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" not in text:
        await update.message.reply_text(
            ...
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=inline_menu(ID_DEL_USUARIO)
        )
        return
...
async def deltask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /deltask <id>", reply_markup=inline_menu(update.effective_user.id)
)
        return
    ...
    await update.message.reply_text("✅ Eliminada." if deleted else "⚠️ No encontré ese ID.", reply_markup=inline_menu(update.effective_user.id)
S)
...
async def toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(u.id):
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /toggle <id>", reply_markup=inline_menu(update.effective_user.id)
)
        return
    task_id = int(context.args[0])
    ...
    await update.message.reply_text(("✅ Activada." if state else "⛔ Pausada.") if state is not None else "⚠️ No encontré ese ID.", reply_markup=inline_menu(update.effective_user.id)
...
async def newcampaign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Campaña creada. ID: {cid}", reply_markup=inline_menu(update.effective_user.id)
)
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}", reply_markup=inline_menu(update.effective_user.id)
)
...
async def endcampaign_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(u.id):
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /endcampaign <id>", reply_markup=inline_menu(update.effective_user.id)
)
        return
    ...
    await update.message.reply_text("✅ Cerrada." if changed else "⚠️ No existe.", reply_markup=inline_menu(update.effective_user.id)
)
...
async def paywithdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /paywithdraw <id> (opcional: nota)", reply_markup=inline_menu(update.effective_user.id)
)
        return
    ...
    if status == "not_found":
        await update.message.reply_text("⚠️ No existe ese retiro.", reply_markup=inline_menu(update.effective_user.id)
); return
    if status == "not_pending":
        await update.message.reply_text("⚠️ Ese retiro no está pendiente.", reply_markup=inline_menu(update.effective_user.id)
); return
    if status == "insufficient_held":
        await update.message.reply_text("⚠️ No hay retenido suficiente.", reply_markup=inline_menu(update.effective_user.id)
 return
    if status != "ok":
        await update.message.reply_text("⚠️ No pude completar.", reply_markup=inline_menu(update.effective_user.id)
); return

    await update.message.reply_text("✅ Marcado como pagado y retenido descontado.", reply_markup=inline_menu(update.effective_user.id)
)
    try:
        await context.bot.send_message(
            chat_id=user_id,
...
async def rejectwithdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usá: /rejectwithdraw <id> <motivo>", reply_markup=inline_menu(update.effective_user.id)
)
        return
    ...
    status, user_id, amount = await asyncio.to_thread(_work)
    if status == "not_found":
        await update.message.reply_text("⚠️ No existe.", reply_markup=inline_menu(update.effective_user.id)
); return
    if status == "not_rejectable":
        await update.message.reply_text("⚠️ No se puede rechazar en este estado.", reply_markup=inline_menu(update.effective_user.id)
); return
    if status != "ok":
        await update.message.reply_text("⚠️ No pude completar.", reply_markup=inline_menu(update.effective_user.id)
); return

    await update.message.reply_text("✅ Rechazado y devuelto a disponible.", reply_markup=inline_menu(update.effective_user.id)
)
    try:
        await context.bot.send_message(
            chat_id=user_id,
...
async def on_menu_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu:home":
        await safe_edit_or_send(q, "📌 **Menú**\nElegí una opción 👇", inline_menu(update.effective_user.id))
        return
